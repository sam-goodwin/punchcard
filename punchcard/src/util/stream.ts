...
export type EventType<E extends Stream<any, any, any, any>> = E extends Stream<
  infer E,
  any,
  any,
  any
>
  ? E
  : never;
export type DataType<E extends Stream<any, any, any, any>> = E extends Stream<
  any,
  infer I,
  any,
  any
...
export type DependencyType<
  E extends Stream<any, any, any, any>
> = E extends Stream<any, any, infer D, any> ? D : never;
 ...
 */
export abstract class Stream<E extends RecordShape, T, D extends any[], C> {
  constructor(
  ...
  public map(inputOrHandle: any, handle?: any): any {
    if (typeof inputOrHandle === "function") {
      ...
      return this.flatMap({depends: inputOrHandle.depends}, async (v, c) => [
        await handle(v, c),
      ]);
    }
  }
  ...
  public flatMap<U, D2 extends Dependency<any> | undefined>(
    ...
    return this.chain({
      depends: (input.depends === undefined
        ? this.dependencies
        : [input.depends].concat(this.dependencies)) as any,
   ...
   * @param dependencies - todo: add description
   */
  ...
  public run(event: Value.Of<E>, deps: Clients<D>): AsyncIterableIterator<T> {
    if (this.dependencies === this.previous.dependencies) {
      return this.f(
        this.previous.run(event, deps),
        deps ? (deps as any[])[0] : undefined,
      );
    } else {
      ...
      return this.f(
        this.previous.run(event, deps ? deps.slice(1) : ([] as any)),
        deps ? (deps as any[])[0] : undefined,
      );
    }
  }
  ...
  public forEach<D2 extends Dependency<any> | undefined>(
    ...
    E,
    any,
    D2 extends undefined ? Dependency.Concat<D> : Dependency.Concat<Cons<D, D2>>
  > {
    // TODO: let the stream type determine default executor service
    const executorService =
      (input.config && input.executorService) ||
      new Lambda.ExecutorService({
        timeout: Duration.seconds(10),
      });
    const l = executorService.spawn(
      scope,
      id,
      {
        depends:
          input.depends === undefined
      ...
      async (event: any, deps) => {
        if (input.depends === undefined) {
          ...
          for await (const value of this.run(
            event,
            (deps as any[]).slice(1) as Clients<D>,
          )) {
    ...
    l.resource.chain((l) =>
      ((input.config || Build.of(undefined)) as Build<
        C | undefined
      >).chain((config) =>
        this.eventSource(config).map((es) => l.addEventSource(es)),
      ),
    );
    return l as any;
  }
