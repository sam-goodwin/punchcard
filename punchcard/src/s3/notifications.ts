 ...
 */
export class Notifications<T, D extends any[]> extends Stream<
  typeof Event.Payload,
  T,
  D,
  events.S3EventSourceProps
> {
  constructor(
    public readonly bucket: Bucket,
    previous: Notifications<any, any>,
    input: {
      depends: D;
      handle: (
   ...
   */
  public eventSource(
    props?: events.S3EventSourceProps,
  ): Build<events.S3EventSource> {
    return CDK.chain(({lambdaEventSources, s3}) =>
      this.bucket.resource.map((bucket) => {
        return new lambdaEventSources.S3EventSource(
          bucket,
          props || {
