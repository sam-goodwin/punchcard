...
export class $DynamoDBUtil {
   ...
   */
  public toDynamoDB<T extends GraphQL.Type>(
    _object: T,
  ): GraphQL.TypeOf<ToDynamoDBJson<GraphQL.ShapeOf<T>>> {
  ...
  public toBoolean(
    _bool: GraphQL.Bool,
  ): GraphQL.TypeOf<ToDynamoDBJson<BoolShape>> {
    throw new Error("todo");
  }

  public toNumber<N extends GraphQL.Integer | GraphQL.Number>(
    _number: N,
  ): GraphQL.TypeOf<ToDynamoDBJson<GraphQL.ShapeOf<N>>> {
    throw new Error("todo");
  }

  public toString(
    _value: GraphQL.String,
  ): GraphQL.TypeOf<ToDynamoDBJson<StringShape>> {
    throw new Error("todo");
  }

  public toStringSet(
    _value: GraphQL.List<GraphQL.String>,
  ): GraphQL.TypeOf<
    RecordType<{
 ...
 */
export type ToDynamoDBJson<T extends Shape> = T extends NumericShape
  ? RecordType<{S: StringShape}>
  : T extends StringShape
  ? RecordType<{S: StringShape}>
  : T extends TimestampShape
  ? RecordType<{S: TimestampShape}>
  : T extends BoolShape
  ? RecordType<{BOOL: BoolShape}>
  : T extends BinaryShape
  ? RecordType<{B: StringShape}>
