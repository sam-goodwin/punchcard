import {RecordType, Shape, Value} from "@punchcard/shape";
...
> {
  handle: (
    request: Value.Of<T>,
    runtimeContext: Client<R>,
  ) => Promise<Response<U, keyof U>>;
...
export type MappingType<T> = T extends Shape
  ? TypedMapping<T>
...
export type RequestMappings<
  S extends RecordType,
  M extends MethodName
> = M extends "GET" // 'GET' methods require mappings for all properties since there is no body
