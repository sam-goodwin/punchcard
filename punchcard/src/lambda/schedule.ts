...
export interface ScheduleProps<D extends Dependency<any>>
  extends FunctionProps<typeof CloudWatch.Event.Payload, any, D> {
  schedule: Schedule;
}
...
export function schedule<D extends Dependency<any>>(
  ...
  const f = new Function<typeof CloudWatch.Event.Payload, any, D>(
    scope,
    id,
    props,
    handler,
  ...
  CDK.chain(({events, eventsTargets}) =>
    f.resource.map(
      (f) =>
...
export interface CronOptions {
   ...
   *
   * @defaultValue - Every day of the month
   */
  readonly day?: string;
   ...
   *
   * @defaultValue - Any day of the week
   */
 ...
 */
function maybeRate(interval: number, singular: string): string | undefined {
 ...
 */
function makeRate(interval: number, singular: string): string {
  return interval === 1
    ? `rate(1 ${singular})`
