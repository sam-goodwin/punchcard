// Copied from @aws-cdk/core to avoid depending on the whole CDK framework just for durations.
...
export class Duration {
  ...
  public toCDKDuration(): cdk.Duration {
    return new (Build.resolve(CDK).core.Duration as any)(
      this.amount,
      this.unit,
    );
  }
  ...
  private fractionDuration(
    symbol: string,
    modulus: number,
    next: (amount: number) => Duration,
  ): string {
    if (this.amount < modulus) {
      return `${this.amount}${symbol}`;
    }
    const remainder = this.amount % modulus;
    const quotient = next((this.amount - remainder) / modulus)
      .toISOString()
      .slice(2);
    return remainder > 0 ? `${quotient}${remainder}${symbol}` : quotient;
  }
}
...
class TimeUnit {
  ...
  private constructor(
    ...
    public readonly inSeconds: number,
  ) {}
