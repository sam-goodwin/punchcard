...
import {StatementF, call} from "../appsync/intepreter/statement";
 ...
 */
export type TableOverrideProps = Omit<
  dynamodb.TableProps,
...
> extends Construct implements Resource<dynamodb.Table> {
  ...
  constructor(scope: Scope, id: string, props: TableProps<DataType, Key>) {
    ...
    this.key = props.key;
    const [partitionKeyName, sortKeyName] = getKeyNames<
      Shape.Resolve<DataType>
    >(props.key);

    this.resource = CDK.chain(({dynamodb}) =>
      Scope.resolve(scope).map((scope) => {
        const extraTableProps = props.tableProps
          ? Build.resolve(props.tableProps)
          : {};
  ...
  public get(
    _key: KeyGraphQLRepr<Shape.Resolve<DataType>, Key>,
  ): StatementF<GraphQL.TypeOf<Shape.Resolve<DataType>>> {
    // @ts-ignore
    return call(this);
  }
   ...
   */
  public projectTo<Projection extends RecordShape>(
    projection: AssertValidProjection<Shape.Resolve<DataType>, Projection>,
  ...
  ): Dependency<TableClient<Shape.Resolve<DataType>, Key>> {
    return {
      bootstrap: Run.of(
        (ns, cache): Promise<TableClient<Shape.Resolve<DataType>, Key>> =>
          Promise.resolve(
            new TableClient({
      ...
      install: this.resource.map((table) => (ns, grantable): void => {
...
export type KeyGraphQLRepr<
  DataType extends RecordShape,
  K extends DDB.KeyOf<DataType>
> = {
...
export namespace Table {
  ...
  export type ReadOnly = Omit<
    TableClient<Shape.Resolve<A>, K>,
    "put" | "putBatch" | "delete" | "update"
  >;
   ...
   */
  export type WriteOnly = Omit<
    TableClient<Shape.Resolve<A>, K>,
    "batchGet" | "get" | "scan" | "query"
  >;
...
export namespace Table {
  export type Data<T extends Table<any, any>> = T extends Table<infer D, any>
    ? Shape.Resolve<D>
    : never;
  export type Key<T extends Table<any, any>> = T extends Table<any, infer K>
    ? K
    : never;
}

type AssertValidProjection<
  T extends RecordShape,
  P extends RecordShape
> = T["Members"] extends P["Members"] ? P : never;
