...
export interface Integration<_R extends Context> extends api.LambdaIntegration {
  mapResource(resource: Resource): void;
  findResource(resourceId: string): Resource;
}

const resourceIdPrefix = 'resource_id_';
export class LambdaIntegration<R extends Context> extends api.LambdaIntegration implements Integration<R> {
