...
export namespace HashCode {
  ...
  export class Visitor implements ShapeVisitor<HashCode<any>> {
        ...
        switch (typeof value) {
          ...
          case "object":
            if (Array.isArray(value)) {
              ...
              let result = 1;
              value.forEach(
                (item) => (result += prime * result + hashCode(item)),
              );
              return result;
            } else {
              const prime = 31;
    ...
    public numberShape(_shape: NumberShape): HashCode<any> {
      return <T>(n: T): number => (n as any) as number;
    }
    ...
    public recordShape(shape: RecordShape<any>): HashCode<any> {
      ...
      return ((value: any) => {
        ...
        Object.keys(value).forEach((key) => {
          result += prime * result + stringHashCode(key);
          result += prime * result + fields[key](value[key]);
        });
        return result;
      }) as any;
    }
    ...
    public stringShape(_shape: StringShape): HashCode<any> {
      return <T>(s: T): number => stringHashCode((s as any) as string);
    }
