...
export abstract class Shape {
  ...
  public visit<V extends ShapeVisitor<T, C>, T, C>(visitor: V, context: C): T {
    return visitor[this.Kind](this as any, context);
  }

  public apply<T extends Trait<this, any>>(
    trait: T,
  ): Apply<this, Trait.GetData<T>> {
    return Meta.apply(this, trait[Trait.Data]);
  }
}
export namespace Shape {
  export type Like<T extends Shape = Shape> = T | {Shape: T};
  export type Resolve<T extends Shape.Like> = T extends {Shape: infer S}
    ? S extends never
      ? T
      : S
    : T extends Shape
  ...
  export type Infer<T> = T extends boolean
    ? BoolShape
    : T extends Buffer
    ? BinaryShape
    : T extends Date
