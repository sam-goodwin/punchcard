...
export class ToJsonSchemaVisitor implements ShapeVisitor<JsonSchema, undefined> {
  public dynamicShape(_shape: DynamicShape<any>, _context: undefined): AnySchema {
    return {
      type: {}
    };
  }
  public binaryShape(shape: BinaryShape, _context: undefined): BinarySchema<any> {
    return {
  ...
  public boolShape(_shape: BoolShape): BoolSchema {
    return {
      type: 'boolean'
    };
  }
  ...
  public timestampShape(_shape: TimestampShape): TimestampSchema {
    return {
      type: 'string',
      format: 'date-time'
    };
