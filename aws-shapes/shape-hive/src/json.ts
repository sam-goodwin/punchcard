    ...
    return {
      ...
      read: (value: string) => {
        ...
        if (!match) {
          ...
          throw new Error(
            `invalid Hive timestamp format, expected ISO8601 or YYYY-MM-DD HH:mm:ss.SSS, got: ${value}`,
          );
        }
...
function formatHiveDateString(d: Date) {
  return `${pad(d.getUTCFullYear(), 4)}-${pad(d.getUTCMonth() + 1, 2)}-${pad(
    d.getUTCDate(),
    2,
...
export class JsonDataType implements DataType {
  ...
  public mapper<T extends Shape>(type: T): Mapper<Value.Of<T>, Buffer> {
    ...
    return {
      read: (buffer) =>
        jsonMapper.read(JSON.parse(buffer.toString("utf8"))) as any,
      write: (value) =>
        Buffer.from(
          JSON.stringify(jsonMapper.write(value as any)) + "\n",
          "utf8",
        ),
    };
  }
